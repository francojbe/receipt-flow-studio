    ...
    return (
      <section className="flex flex-col h-full pt-2 animate-fade-in items-center justify-center">
        <div className="ios-card p-6 text-center max-w-sm mx-4">
          <h2 className="text-xl font-bold mb-2 text-white">Suscripción Requerida</h2>
    ...
    <section className="flex flex-col h-full pt-2 animate-fade-in">
      ...
      <div className="ios-card p-2 mb-4 relative">
        <div className="flex items-center justify-between px-3 gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-white text-right p-2 capitalize pointer-events-none">
      ...
      <div className="flex-1 ios-card overflow-hidden flex flex-col relative">
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-white/60 uppercase bg-white/10 sticky top-0">
              <tr>
                ...
                <tr>
                  <td colSpan={3} className="px-5 py-10 text-center">
                    <div className="flex justify-center">
                      <div className="loader border-white/30 border-t-white" />
                    </div>
                  </td>
                </tr>
              ) : cartolaData.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-5 py-8 text-center text-white/50 text-xs">
                    Sin movimientos.
                  </td>
                </tr>
              ) : (
                cartolaData.map((row, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-white/5 transition-colors">
                      <td className="px-5 py-3 whitespace-nowrap text-xs font-medium text-white/60">
                        {fecha}
                      </td>
                      <td className="px-5 py-3 text-xs font-medium text-white truncate max-w-[140px]">
                        {desc}
                      </td>
      ...
      <div className="mt-4 pb-4">
        <button
          ...
          className="w-full bg-white hover:bg-white/90 disabled:bg-white/20 disabled:text-white/40 disabled:cursor-not-allowed text-black font-semibold py-4 rounded-full shadow-ios-lg transition-all flex items-center justify-center gap-2 text-sm press-effect"
        >
